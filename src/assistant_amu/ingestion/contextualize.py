...
class ContextCache:
    ...
    replayed for the exact text it was written for.

    Unlike ``chroma_db/`` and ``corpus/raw/``, this file IS versioned even though
    it is derived data: the collections that produced the §5.3.1 reports are
    gone, so it is now their only reproduction path. Excluding it made
    ``eval/repro_contextual_retrieval.py`` free on one machine only.
    """
